...
class TestLists < Test::Unit::TestCase
  ...
  context "When using the foursquare API and working with lists" do
    ...
    should "fetch info for a single list" do
      stub_get("/lists/#{@list_id}?oauth_token=#{@client.oauth_token}", "lists/list.json")
    ...
    should "add a list" do
      stub_post("/lists/add?oauth_token=#{@client.oauth_token}&name=Ramen+Spots", "lists/list_created.json")
      list = @client.add_list(:name => "Ramen Spots")
      list.name.should == "Ramen Spots"
    end

    should "follow a list" do
      stub_post("/lists/#{@list_id}/follow?oauth_token=#{@client.oauth_token}", "lists/list_followed.json")
      list = @client.follow_list(@list_id)
      list.following.should == true
    end

    should "unfollow a list" do
      stub_post("/lists/#{@list_id}/unfollow?oauth_token=#{@client.oauth_token}", "lists/list.json")
      list = @client.unfollow_list(@list_id)
      list.following.should == false
    end

    should "update a list" do
      stub_post("/lists/#{@list_id}/update?oauth_token=#{@client.oauth_token}&name=Ramen+Sports", "lists/list_updated.json")
    ...
    should "add an item to a list" do
      ...
      stub_post("/lists/#{@list_id}/additem?oauth_token=#{@client.oauth_token}&venueId=#{venue_id}", "lists/list_item.json")
    ...
    should "delete an item from a list" do
      ...
      stub_post("/lists/#{@list_id}/deleteitem?oauth_token=#{@client.oauth_token}&itemId=#{item_id}", "lists/list_item.json")
    ...
    should "move an item on a list" do
      ...
      stub_post("/lists/#{@list_id}/moveitem?oauth_token=#{@client.oauth_token}&itemId=#{item_id}&beforeId=#{before_id}", "lists/list_moved_item.json")
