...
class TestSpecials < Test::Unit::TestCase
  ...
  context "When using the foursquare API and working with specials" do
    ...
    should "fetch info for a single special" do
      stub_get("/specials/4bd876f886ba62b58a6e88b3?oauth_token=#{@client.oauth_token}", "specials/special.json")
    ...
    should "fetch results when searching for nearby specials" do
      stub_get("/specials/search?ll=40.7%2C-73.9&oauth_token=#{@client.oauth_token}", "specials/search_specials.json")
    ...
    should "add a special for a venue with given params" do
      stub_post("/specials/add?text=Test&unlockedText=Congrats&oauth_token=#{@client.oauth_token}", "specials/special_created.json")
