...
class TestUsers < Test::Unit::TestCase
  ...
  context "When using the foursquare API and working with users" do
    ...
    should "fetch user's leaderboard" do
      stub_get("/users/leaderboard?oauth_token=#{@client.oauth_token}&neighbors=2", "users/leaderboard.json", )
      leaderboard = @client.leaderboard(:neighbors => 2)
      leaderboard.leaderboard['count'].should == 3
    end

    should "fetch info for a single user" do
      stub_get("/users/self?oauth_token=#{@client.oauth_token}", "users/user.json")
    ...
    should "fetch results when searching for users" do
      stub_get("/users/search?oauth_token=#{@client.oauth_token}&twitter=matt_mueller", "users/search_users.json")
    ...
    should "fetch tips" do
      stub_get("/users/self/tips?oauth_token=#{@client.oauth_token}", "users/user_tips.json")
    ...
    should "fetch tips and filter with term #{QUERY}" do
      stub_get("/users/self/tips?oauth_token=#{@client.oauth_token}&query=#{QUERY}", "users/user_tips.json")
      tips = @client.user_tips('self', {:query => QUERY})
      tips.items.size.should == 1
    end

    should "fetch lists for a single user" do
      stub_get("/users/self/lists?oauth_token=#{@client.oauth_token}", "users/user_lists.json")
    ...
    should "list only user with tip #{QUERY}" do
      stub_get("/tips/search?oauth_token=#{@client.oauth_token}&ll=77.7%2C-77.7&query=#{QUERY}&limit=500", "tips/search_tips.json")
    ...
    should "find a users mayorships" do
      stub_get("/users/self/mayorships?oauth_token=#{@client.oauth_token}", "users/user_mayorships.json")
    ...
    should "find a users checkins - plain (without optional params)" do
      stub_get("/users/self/checkins?oauth_token=#{@client.oauth_token}", "users/user_checkins_plain.json")
      checkins = @client.user_checkins
      checkins.items.size.should == 20
    end

    should "find a users checkins with afterTimestamp param" do
      stub_get("/users/self/checkins?oauth_token=#{@client.oauth_token}&limit=10&offset=2&sort=newestfirst&afterTimestamp=1279044824", "users/user_checkins_aftertimestamp.json")
    ...
    should "find a users checkins with beforeTimestamp param" do
      stub_get("/users/self/checkins?oauth_token=#{@client.oauth_token}&limit=5&offset=20&sort=oldestfirst&beforeTimestamp=1355867689", "users/user_checkins_beforetimestamp.json")
    ...
    should "find a users venuestats by self" do
      stub_get("/users/self/venuestats?oauth_token=#{@client.oauth_token}", "users/user_venuestats.json")
    ...
    should "find a users venuestats by inputting friends user_id" do
      stub_get("/users/555555/venuestats?oauth_token=#{@client.oauth_token}", "users/user_venuestats_friend_id.json")
    ...
    should "fetch photos" do
      stub_get("/users/self/photos?oauth_token=#{@client.oauth_token}", "users/user_photos.json")
