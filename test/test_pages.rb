...
class TestPages < Test::Unit::TestCase
  ...
  context "When using the foursquare API and working with pages" do
    ...
    should "fetch info for a single page" do
      stub_get("/pages/14046309?oauth_token=#{@client.oauth_token}", "pages/page.json")
    ...
    should "fetch results when searching for a page" do
      stub_get("/pages/search?name=NashvilleGeeks&oauth_token=#{@client.oauth_token}", "pages/search_pages.json")
    ...
    should "fetch venues for a given page" do
      stub_get("/pages/1070527/venues?limit=2&oauth_token=#{@client.oauth_token}", "pages/page_venues.json")
